...
export function QuestionDetailView() {
  ...
  const nextQuestion = index >= 0 && index < QUESTIONS.length - 1 ? QUESTIONS[index + 1] : null;
  ...
  const toggleBookmark = () => {
    ...
    const updated = { ...userState, bookmarks: nextBookmarks };
      ...
      <Card className="quant-card">
        <div className="quant-grid">
          ...
          <div className="quant-actions">
            ...
            <Button onClick={() => markQuestion('CORRECT')} disabled={Boolean(result)}>
              Correct
            </Button>
            <Button onClick={() => markQuestion('PARTIAL')} disabled={Boolean(result)}>
              Partial
            </Button>
            <Button onClick={() => markQuestion('INCORRECT')} disabled={Boolean(result)}>
              Incorrect
            </Button>
          </div>
          ...
          <div className="quant-nav-row">
            <Button
              ...
              onClick={() => previousQuestion && navigate(`/quant/questions/${previousQuestion.id}`)}
            >
              ← Back
            </Button>
            <Button
              ...
              onClick={() => nextQuestion && navigate(`/quant/questions/${nextQuestion.id}`)}
            >
              → Next
            </Button>
          </div>
