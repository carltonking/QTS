...
export default function ChessModule() {
  ...
  const [playerRating, setPlayerRating] = useLocalStorage<PlayerRating>(
    STORAGE_KEYS.CHESS_PLAYER_RATING,
    defaultPlayerRating(),
  );
  const [attemptBuffer, setAttemptBuffer] = useLocalStorage<PuzzleAttempt[]>(
    STORAGE_KEYS.CHESS_ATTEMPTS,
    [],
  );
  const [periodPuzzles, setPeriodPuzzles] = useState<Puzzle[]>([]);
  const [stats, setStats] = useLocalStorage<ChessStats>(
    STORAGE_KEYS.CHESS_STATS,
    defaultStats(),
  );
  ...
  const [hintsEnabled, setHintsEnabled] = useLocalStorage<boolean>(
    STORAGE_KEYS.CHESS_HINTS,
    true,
  );
  const { syncChessRating, syncChessAttempt } = useSyncToBackend();
  ...
  const processCompletedPuzzle = async (
    score: number,
    timeTaken: number,
    _attempts: number,
  ) => {
    ...
    nextStats.accuracy =
      nextStats.total > 0
        ? Math.round((nextStats.firstTry / nextStats.total) * 100)
        : 0;
    ...
    if (pendingAttempts.length >= RATING_PERIOD) {
      const { player } = processRatingPeriod(
        playerRating,
        pendingAttempts,
        pendingPuzzles,
      );
      ...
      setAttemptBuffer([]);
      setPeriodPuzzles([]);
      syncChessRating(player.r, player.rd, nextStats.solved);
    } else {
    ...
    syncChessAttempt(nextAttempt.puzzleId, nextAttempt.score > 0, 0);
    await loadPuzzle();
  };
    ...
    <div className="chess-module">
      ...
      <div className="chess-column">
        <Card>
          ...
          {ratingDeltaText ? (
            <div className="chess-feedback">{ratingDeltaText}</div>
          ) : null}
          <div className="chess-period">
            RATING PERIOD: {attemptBuffer.length} / {RATING_PERIOD}
          </div>
        </Card>
      </div>
    </div>
