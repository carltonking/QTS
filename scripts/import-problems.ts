...
const NEETCODE_150: Record<string, string[]> = {
...
function mapDifficulty(difficulty: RawProblem['difficulty']) {
  if (difficulty === 'Easy') return Difficulty.EASY;
  if (difficulty === 'Medium') return Difficulty.MEDIUM;
  return Difficulty.HARD;
}

function getNeetcodePlacement(slug: string) {
  for (const [category, slugs] of Object.entries(NEETCODE_150)) {
    ...
    if (order !== -1) {
      return {
        ...
        neetcodeOrder: order + 1,
      };
    }
  }

  return {
    neetcodeCategory: null,
    neetcodeOrder: null,
  };
}
...
async function main() {
  ...
  const problemsDir = path.join(rootDir, 'data', 'problems');
  const files = (await fs.readdir(problemsDir)).filter((file) => file.endsWith('.json'));
  let imported = 0;

  for (const file of files) {
    const raw = await fs.readFile(path.join(problemsDir, file), 'utf8');
    const problem = JSON.parse(raw) as RawProblem;
    const placement = getNeetcodePlacement(problem.slug);
    ...
    await prisma.problem.upsert({
      ...
      create: {
        ...
        acceptanceRate: 0,
        neetcodeCategory: placement.neetcodeCategory,
        neetcodeOrder: placement.neetcodeOrder,
      },
      update: {
        title: problem.title,
        difficulty: mapDifficulty(problem.difficulty),
    ...
    imported += 1;
    console.log(`Imported ${imported}/${files.length} problems`);
  }
